'use strict';

var core = require('@tauri-apps/api/core');

/**
 * Check whether the window is desktop underlay.
 *
 * @param label - The label of the window. If not provided, the current window
 *  will be used.
 * @returns Whether the window is desktop underlay.
 */
async function isDesktopUnderlay(label) {
    return await core.invoke("plugin:desktop-underlay|is_desktop_underlay", {
        label,
    });
}
/**
 * Set the window as desktop underlay or revert it to normal.
 *
 * @param desktopUnderlay If `true`, the window will be set as desktop underlay
 *  (no-op if it already is). If `false`, the window will be reverted to normal
 *  (no-op if it is not yet desktop underlay).
 * @param label The label of the window. If not provided, the current window
 *  will be used.
 */
async function setDesktopUnderlay(desktopUnderlay, label) {
    return await core.invoke("plugin:desktop-underlay|set_desktop_underlay", {
        desktopUnderlay,
        label,
    });
}
/**
 * Toggle the desktop underlay state of the window.
 *
 * @param label The label of the window. If not provided, the current window
 *  will be used.
 * @returns Whether the window is desktop underlay after the operation.
 */
async function toggleDesktopUnderlay(label) {
    return await core.invoke("plugin:desktop-underlay|toggle_desktop_underlay", {
        label,
    });
}

exports.isDesktopUnderlay = isDesktopUnderlay;
exports.setDesktopUnderlay = setDesktopUnderlay;
exports.toggleDesktopUnderlay = toggleDesktopUnderlay;
